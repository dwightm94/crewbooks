    ...
    <div style={{ fontFamily: "'Outfit', sans-serif", background: "#FFFBF5", color: "#1A1A1A" }}>
      ...
      <nav style={{ position: "fixed", top: 0, width: "100%", zIndex: 100, background: "rgba(255,251,245,0.9)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "14px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <button onClick={() => router.push("/auth/login")} style={{ padding: "8px 16px", background: "transparent", border: "1px solid #ddd", borderRadius: 10, fontWeight: 600, fontSize: 14, cursor: "pointer", color: "#333", fontFamily: "inherit" }}>Log In</button>
            <button onClick={() => router.push("/auth/signup")} style={{ padding: "8px 16px", background: "linear-gradient(135deg, #F59E0B, #EF4444)", border: "none", borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: "pointer", color: "white", fontFamily: "inherit" }}>Start Free</button>
          </div>
        </div>
      </nav>
      ...
      <section style={{ paddingTop: 120, paddingBottom: 60, textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto", padding: "0 24px", position: "relative" }}>
          ...
          <div className="fade-in fade-in-d3" style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <button onClick={() => router.push("/auth/signup")} style={{ padding: "14px 32px", background: "linear-gradient(135deg, #F59E0B, #EF4444)", border: "none", borderRadius: 14, fontWeight: 800, fontSize: 16, cursor: "pointer", color: "white", boxShadow: "0 4px 20px rgba(245,158,11,0.3)", fontFamily: "inherit" }}>
              Start Free — No Credit Card
            </button>
            <a href="#features" style={{ padding: "14px 24px", background: "white", border: "2px solid #E5E7EB", borderRadius: 14, fontWeight: 700, fontSize: 16, cursor: "pointer", color: "#333", textDecoration: "none", display: "inline-block", fontFamily: "inherit" }}>
              See Features
            </a>
          </div>
      ...
      <section id="pricing" style={{ padding: "60px 24px", maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
          <div className="hover-lift" style={{ background: "white", borderRadius: 20, padding: 28, border: "1px solid rgba(0,0,0,0.08)" }}>
            ...
            <button onClick={() => router.push("/auth/signup")} style={{ width: "100%", padding: "12px", background: "#F3F4F6", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 14, cursor: "pointer", color: "#333", fontFamily: "inherit" }}>Get Started</button>
            <ul style={{ marginTop: 20, listStyle: "none", padding: 0 }}>
              {["3 active jobs", "3 invoices/month", "Crew scheduling", "Photo documentation", "Daily logs", "Estimates & bidding", "Compliance tracking"].map(f => (
                <li key={f} style={{ padding: "6px 0", fontSize: 14, color: "#555", display: "flex", alignItems: "center", gap: 8 }}><span style={{ color: "#22C55E", fontWeight: 700 }}>✓</span>{f}</li>
              ))}
            </ul>
          </div>
          <div className="hover-lift" style={{ background: "white", borderRadius: 20, padding: 28, border: "2px solid #F59E0B", position: "relative" }}>
            ...
            <button onClick={() => router.push("/auth/signup")} style={{ width: "100%", padding: "12px", background: "linear-gradient(135deg, #F59E0B, #EF4444)", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 14, cursor: "pointer", color: "white", fontFamily: "inherit" }}>Start Free Trial</button>
            <ul style={{ marginTop: 20, listStyle: "none", padding: 0 }}>
              {["Everything in Free, plus:", "Unlimited jobs", "Unlimited invoicing", "Online payments (2.5% fee)", "Full reports & analytics", "Payment reminders", "Priority support"].map((f, i) => (
                <li key={f} style={{ padding: "6px 0", fontSize: 14, color: i === 0 ? "#F59E0B" : "#555", fontWeight: i === 0 ? 700 : 400, display: "flex", alignItems: "center", gap: 8 }}>{i > 0 && <span style={{ color: "#22C55E", fontWeight: 700 }}>✓</span>}{f}</li>
              ))}
            </ul>
          </div>
          <div className="hover-lift" style={{ background: "white", borderRadius: 20, padding: 28, border: "1px solid rgba(0,0,0,0.08)", opacity: 0.7 }}>
            ...
            <ul style={{ marginTop: 20, listStyle: "none", padding: 0 }}>
              {["Everything in Pro, plus:", "Up to 10 users", "25GB photo storage", "QuickBooks sync", "Dedicated support"].map((f, i) => (
                <li key={f} style={{ padding: "6px 0", fontSize: 14, color: i === 0 ? "#6366F1" : "#555", fontWeight: i === 0 ? 700 : 400, display: "flex", alignItems: "center", gap: 8 }}>{i > 0 && <span style={{ color: "#22C55E", fontWeight: 700 }}>✓</span>}{f}</li>
              ))}
            </ul>
          </div>
        </div>
      ...
      <section style={{ padding: "60px 24px", textAlign: "center" }}>
        <div style={{ maxWidth: 600, margin: "0 auto", background: "linear-gradient(135deg, #1A1A1A, #333)", borderRadius: 24, padding: "48px 32px" }}>
          ...
          <button onClick={() => router.push("/auth/signup")} style={{ padding: "14px 32px", background: "linear-gradient(135deg, #F59E0B, #EF4444)", border: "none", borderRadius: 14, fontWeight: 800, fontSize: 16, cursor: "pointer", color: "white", fontFamily: "inherit" }}>
            Get Started Free
          </button>
        </div>
      </section>
